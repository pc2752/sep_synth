...
def wave_archi(inputs, backing, guide, is_train):
    ...
    for i in range(config.wavenet_layers):
        skip, residual = wavenet_block(residual,back_embedding, is_train, dilation_rate = config.dilation_rates[i], name = "npss_block_"+str(i+1))
        skips.append(skip)
    for skip in skips:
        output+=skip

    conditioning = tf.layers.batch_normalization(tf.layers.conv1d(back_embedding, config.skip_filters, 1, dilation_rate = 1, padding = 'valid', name = "cond"), training = is_train)
...
def embedding_network(inputs, is_train):
...
def encoder_conv_block(inputs, layer_num, is_train, num_filters = config.filters):
    ...
    output = tf.layers.batch_normalization(tf.nn.relu(tf.layers.conv2d(inputs, num_filters * 2**int(layer_num/4), (config.filter_len,1)
...
def nr_wavenet_block(conditioning, dilation_rate = 2):
    ...
    con_sig_forward = tf.layers.conv1d(con_pad_forward, config.embedding_filters, 2, dilation_rate = dilation_rate, padding = 'valid')
    con_sig_backward = tf.layers.conv1d(con_pad_backward, config.embedding_filters, 2, dilation_rate = dilation_rate, padding = 'valid')
    # con_sig = tf.layers.conv1d(conditioning,config.wavenet_filters,1)

    sig = tf.sigmoid(con_sig_forward+con_sig_backward)


    con_tanh_forward = tf.layers.conv1d(con_pad_forward, config.embedding_filters, 2, dilation_rate = dilation_rate, padding = 'valid')
    con_tanh_backward = tf.layers.conv1d(con_pad_backward, config.embedding_filters, 2, dilation_rate = dilation_rate, padding = 'valid')    
...
def nr_wavenet(inputs, num_block = config.wavenet_layers):
    ...
    prenet_out = tf.layers.dense(inputs, config.embedding_filters)
    # prenet_out = tf.layers.dense(prenet_out, config.wavenet_filters)

    receptive_field = 2**num_block

    first_conv = tf.layers.conv1d(prenet_out, config.embedding_filters, 1)
    skips = []
    ...
    output = tf.layers.conv1d(output,config.embedding_filters,1)
...
def main():    
    ...
    with tf.variable_scope('full_Model') as scope:
        out_put = wave_archi(vec, vec, is_train)
    sess = tf.Session()
