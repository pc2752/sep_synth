...
class MultiSynth(Model):
    ...
    def read_hdf5_file(self, file_name):
        ...
        mix_file = h5py.File(config.backing_dir + file_name)

        voc_stft = np.array(mix_file['mix_stft'])[()]
    ...
    def test_file_hdf5(self, file_name, file_name_singer):
        ...
        singer_name = file_name_singer.split('_')[1]
        singer_index = config.singers.index(singer_name)

        out_feats = self.process_file(voc_stft, singer_index, sess)
    ...
    def process_file(self, voc_stft, singer_id, sess):
        ...
        for in_batch_stft in in_batches_stft :
            feed_dict = {self.input_placeholder: in_batch_stft,self.input_placeholder_singer: in_batch_stft, self.singer_labels: np.ones(config.batch_size)*singer_id, self.is_train: False}
    ...
    def model(self):
        ...
        with tf.variable_scope('Final_Model') as scope:
            self.output = modules.full_network(self.f0_probs, self.pho_probs, self.singer_probs, self.is_train)
